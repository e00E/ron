...
pub enum Error {
    ...
    Message(String),
    // FIXME: serde_json boxed this to keep the error size down, should we?
    Io(io::Error),
}
...
impl<W: io::Write> Serializer<W> {
    ...
    pub fn new(mut writer: W, config: Option<PrettyConfig>, struct_names: bool) -> Result<Self> {
        if let Some(conf) = &config {
            if conf.extensions.contains(Extensions::IMPLICIT_SOME) {
                writer.write(b"#![enable(implicit_some)]")?;
                writer.write(conf.new_line.as_bytes())?;
            };
        };
        Ok(Serializer {
            output: writer,
    ...
    fn start_indent(&mut self) -> Result<()> {
        if let Some((ref config, ref mut pretty)) = self.pretty {
            ...
            if pretty.indent < config.depth_limit {
                ...
                if !is_empty {
                    self.output.write(config.new_line.as_bytes())?;
                }
            }
        }
        Ok(())
        ...
        if let Some((ref config, ref pretty)) = self.pretty {
            if pretty.indent < config.depth_limit {
                for _ in 0..pretty.indent {
                    self.output.write(config.indentor.as_bytes())?;
                }
            }
        }
        Ok(())
    ...
    fn serialize_escaped_str(&mut self, value: &str) -> io::Result<()> {
        ...
        for c in value.chars().flat_map(|c| c.escape_debug()) {
            self.output.write(c.encode_utf8(&mut scalar).as_bytes())?;
        }
        self.output.write(b"\"")?;
        Ok(())
    }
...
impl<'a, W: io::Write> ser::Serializer for &'a mut Serializer<W> {
    ...
    fn serialize_i64(self, v: i64) -> Result<()> {
        // TODO optimize
        self.output.write(&v.to_string().as_bytes())?;
        Ok(())
    }
    ...
    fn serialize_char(self, v: char) -> Result<()> {
        ...
        let mut scalar = [0u8; 4];
        self.output.write(v.encode_utf8(&mut scalar).as_bytes())?;
        self.output.write(b"'")?;
        Ok(())
    }
    ...
    fn serialize_unit_variant(self, _: &'static str, _: u32, variant: &'static str) -> Result<()> {
        self.output.write(variant.as_bytes())?;
        ...
        T: ?Sized + Serialize,
    {
        ...
        if let Some((ref config, ref mut pretty)) = ser.pretty {
            if pretty.indent < config.depth_limit {
                ...
                ser.output.write(config.new_line.as_bytes())?;
            }
        }
        ser.indent()?;
        ...
        let ser = match self {
            ...
            Compound::Map { ser, .. } => {
                ...
                if let Some((ref config, ref pretty)) = ser.pretty {
                    if pretty.indent < config.depth_limit {
                        ser.output.write(if ser.separate_tuple_members() {
